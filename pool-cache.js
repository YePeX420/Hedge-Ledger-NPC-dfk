// pool-cache.js
//
// Simple in-memory cache for DFK garden pools.
//
// Source of truth: onchain-data.getGardenPools('dfk')
//
// Exports:
//
...
// --------------------------
// Internal: build cache entry
// --------------------------
function buildPoolCacheEntry(pool) {
  ...
  return {
    ...
    // TVL & APR placeholders (can be enriched by analytics later)
    totalTVL: pool.totalTVL || 'N/A',
...
async function refreshPoolCache() {
  try {
    console.log('[pool-cache] Refreshing pool cache from onchain-data...');
    ...
    const entries = dfkPools.map(buildPoolCacheEntry);
    ...
    console.log(`[pool-cache] ✓ Cache refreshed with ${entries.length} pools.`);
    return CACHE;
  } catch (err) {
