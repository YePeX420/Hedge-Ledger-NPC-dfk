...
async function startAdminWebServer() {
  ...
  // a dashboard user session where allowedTabs includes ANY of the given tabIds
  function isAdminOrHasTab(...tabIds) {
    return async function(req, res, next) {
      try {
        ...
        // Try dashboard user session with tab permission (any of tabIds)
        ...
        if (userToken) {
          ...
          if (userSession && new Date(userSession.expiresAt) >= new Date()) {
            ...
            if (user && user.isActive && (!user.expiresAt || new Date(user.expiresAt) >= new Date())) {
              if (Array.isArray(user.allowedTabs) && tabIds.some(t => user.allowedTabs.includes(t))) {
        ...
        console.error(`[TabAuth:${tabIds.join('|')}] error:`, err);
        res.status(500).json({ error: 'Authentication check failed' });
      }
    };
  }
  ...
  app.get("/api/pve/loot-hierarchical/:activityId", async (req, res) => {
    try {
      ...
      const equipmentVariants = await db.execute(sql`
        ...
        FROM pve_reward_events r
        JOIN pve_loot_items l ON r.item_id = l.id
        LEFT JOIN dim_weapon_details w_activity ON r.equipment_type = w_activity.weapon_type_id AND r.display_id = w_activity.display_id AND r.chain_id = w_activity.chain_id AND r.activity_id = w_activity.activity_id
        LEFT JOIN dim_weapon_details w_chain ON r.equipment_type = w_chain.weapon_type_id AND r.display_id = w_chain.display_id AND r.chain_id = w_chain.chain_id AND w_chain.activity_id = 0
        LEFT JOIN dim_weapon_details w_shared ON r.equipment_type = w_shared.weapon_type_id AND r.display_id = w_shared.display_id AND w_shared.chain_id = 0 AND w_shared.activity_id = 0
        LEFT JOIN dim_armor_details a_activity ON r.equipment_type = a_activity.armor_type_id AND r.display_id = a_activity.display_id AND r.chain_id = a_activity.chain_id AND r.activity_id = a_activity.activity_id
        LEFT JOIN dim_armor_details a_chain ON r.equipment_type = a_chain.armor_type_id AND r.display_id = a_chain.display_id AND r.chain_id = a_chain.chain_id AND a_chain.activity_id = 0
        LEFT JOIN dim_armor_details a_shared ON r.equipment_type = a_shared.armor_type_id AND r.display_id = a_shared.display_id AND a_shared.chain_id = 0 AND a_shared.activity_id = 0
        LEFT JOIN dim_accessory_details acc_activity ON r.equipment_type = acc_activity.accessory_type_id AND r.display_id = acc_activity.display_id AND r.chain_id = acc_activity.chain_id AND r.activity_id = acc_activity.activity_id
        LEFT JOIN dim_accessory_details acc_chain ON r.equipment_type = acc_chain.accessory_type_id AND r.display_id = acc_chain.display_id AND r.chain_id = acc_chain.chain_id AND acc_chain.activity_id = 0
        LEFT JOIN dim_accessory_details acc_shared ON r.equipment_type = acc_shared.accessory_type_id AND r.display_id = acc_shared.display_id AND acc_shared.chain_id = 0 AND acc_shared.activity_id = 0
        WHERE r.activity_id = ${activityIdInt} 
          AND r.is_equipment = TRUE
          AND (l.item_type = 'equipment' OR l.item_type IS NULL)
  ...
  app.get('/api/admin/tournament/completed', isAdminOrHasTab('dfk-tournaments', 'previous-tournaments'), async (req, res) => {
  ...
  // GET /api/admin/tournament/bracket/:id — full bracket detail (on-chain)
  {
    ...
    app.get("/api/admin/tournament/bracket/:id", isAdminOrHasTab('dfk-tournaments', 'previous-tournaments'), async (req, res) => {
  ...
  app.post('/api/admin/tournament/bracket/:id/ai-matchup', isAdminOrHasTab('dfk-tournaments', 'previous-tournaments'), async (req, res) => {
    try {
      const tournamentId = parseInt(req.params.id);
      if (isNaN(tournamentId)) return res.status(400).json({ ok: false, error: 'Invalid tournament ID' });
  ...
  app.post('/api/admin/tournament/bracket/:id/rank-teams', isAdminOrHasTab('dfk-tournaments', 'previous-tournaments'), async (req, res) => {
    try {
      const tournamentId = parseInt(req.params.id);
      if (isNaN(tournamentId)) return res.status(400).json({ ok: false, error: 'Invalid tournament ID' });
  ...
  app.post('/api/admin/tournament/bracket/:id/player-tips', isAdminOrHasTab('dfk-tournaments', 'previous-tournaments'), async (req, res) => {
    try {
      const tournamentId = parseInt(req.params.id);
      if (isNaN(tournamentId)) return res.status(400).json({ ok: false, error: 'Invalid tournament ID' });
  ...
  // Auth: exchange DFK_FIREBASE_REFRESH_TOKEN for a Firebase id_token, then use as Bearer.
  const DFK_FIREBASE_API_KEY    = 'AIzaSyDan3VT-fwwsQJV7fQFgsWTv5q6op96K8g';
  const DFK_FIREBASE_PROJECT_ID = 'dfk-user-api';
  const _fbTokenCache = { token: null, expiresAt: 0 };

  async function getFirebaseIdToken() {
    if (_fbTokenCache.token && Date.now() < _fbTokenCache.expiresAt) return _fbTokenCache.token;
    const refreshToken = process.env.DFK_FIREBASE_REFRESH_TOKEN;
    if (!refreshToken) throw new Error('DFK_FIREBASE_REFRESH_TOKEN not set');
    const resp = await fetch(
      `https://securetoken.googleapis.com/v1/token?key=${DFK_FIREBASE_API_KEY}`,
      {
        method: 'POST',
    ...
    if (!resp.ok) {
      ...
      throw new Error(`Firebase token exchange failed: ${resp.status} — ${txt.slice(0, 200)}`);
    }
    const data = await resp.json();
    _fbTokenCache.token = data.id_token;
    _fbTokenCache.expiresAt = Date.now() + 55 * 60 * 1000; // 55 min
  ...
  async function fetchFirestoreSubcollection(idToken, collectionPath) {
    ...
    if (!resp.ok) {
      const txt = await resp.text();
      throw new Error(`Firestore fetch failed: ${resp.status} — ${txt.slice(0, 300)}`);
    }
    return resp.json();
  }
  ...
  // Persists confirmed ID back to DB when found.
  async function fetchBattleLogForBout(tournamentId, boutRow) {
  ...
  app.get('/api/admin/firebase/battle-log-probe', isAdmin, async (req, res) => {
    try {
      ...
      if (battleId) {
        // Direct probe with an explicit battle ID
        const data = await fetchFirestoreSubcollection(idToken, `battles/${battleId}/battle-logs`);
        const docs = (data.documents || []).map(parseFirestoreDoc).filter(Boolean);
        const fieldKeys = docs.length > 0 ? Object.keys(docs[0]).filter(k => k !== '_id') : [];
        return res.json({ ok: true, battleId, rawDocCount: docs.length, fieldKeys, firstDoc: docs[0] ?? null });
      }

      // No battleId — list the battles collection to understand ID format
      const base = `https://firestore.googleapis.com/v1/projects/${DFK_FIREBASE_PROJECT_ID}/databases/combat-engine/documents`;
      const listResp = await fetch(`${base}/battles?pageSize=20`, {
        headers: { Authorization: `Bearer ${idToken}` },
      });
      if (!listResp.ok) {
        const txt = await listResp.text();
        return res.status(listResp.status).json({ ok: false, error: `List failed: ${txt.slice(0, 300)}` });
      }
  ...
  app.get('/api/admin/tournament/bracket/:id/bout-battle-log', isAdminOrHasTab('dfk-tournaments', 'previous-tournaments'), async (req, res) => {
    try {
      ...
      const [bout] = await rp.unsafe(`
        SELECT id, tournament_id, round_number, match_index, player_a, player_b,
               winner_address, is_complete, firebase_battle_id
  ...
  app.get('/api/admin/tournament/:id/firebase-direct-log', isAdminOrHasTab('dfk-tournaments', 'previous-tournaments'), async (req, res) => {
    try {
      const tournamentId = String(req.params.id);
  ...
  app.get('/api/admin/tournament/:id/scan-bout-for-players', isAdminOrHasTab('dfk-tournaments', 'previous-tournaments'), async (req, res) => {
    try {
      const tournamentId = String(req.params.id);
  ...
  app.get('/api/admin/tournament/bracket/:id/matchup-history', isAdminOrHasTab('dfk-tournaments', 'previous-tournaments'), async (req, res) => {
    try {
      const tournamentId = parseInt(req.params.id);
      if (isNaN(tournamentId)) return res.status(400).json({ ok: false, error: 'Invalid tournament ID' });
  ...
  app.post('/api/admin/tournament/bracket/:id/bout-analysis', isAdminOrHasTab('dfk-tournaments', 'previous-tournaments'), async (req, res) => {
    try {
      const tournamentId = String(req.params.id);
      ...
      const [bout] = await rp.unsafe(`
        SELECT id, tournament_id, tournament_name, round_number, match_index,
               player_a, player_b, player_a_name, player_b_name,
               winner_address, is_complete, tournament_format, firebase_battle_id
        FROM dfk_tournament_bouts
...
In 4-5 sentences explain this upset specifically: (1) HOW did the underdog ${_underdogName} overcome the stat disadvantage — cite specific class skills, passive synergies, or survivability traits that neutralized the favorite's edge. (2) WHERE did ${_favoriteName} make critical mistakes — poor target priority, failure to use consumables at the right moment, or stat weaknesses exploited by the opponent. (3) What was the key turning-point or momentum shift (reference turn numbers and HP values if battle log data is available). Be concrete and tactical.`;
  ...
  app.post('/api/admin/tournament/bracket/:id/bout-live-coach', isAdminOrHasTab('dfk-tournaments', 'previous-tournaments'), async (req, res) => {
    try {
      const tournamentId = String(req.params.id);
  ...
  app.get("/api/admin/tournament/:id/comp-data", isAdminOrHasTab('dfk-tournaments', 'previous-tournaments'), async (req, res) => {
    try {
      const { rawPg } = await import('./server/db.js');
      const tournamentId = parseInt(req.params.id);
  ...
  app.get("/api/admin/tournament/:id", isAdminOrHasTab('dfk-tournaments', 'previous-tournaments'), async (req, res) => {
    try {
      const tournamentId = parseInt(req.params.id);
...
`;
  ...
  // ============================================================================
  // TOURNAMENT PREDICTION ENGINE
  // ============================================================================

  // GET /api/admin/tournament/:id/predict — run combat formula on indexed bout
  app.get('/api/admin/tournament/:id/predict', isAdminOrHasTab('dfk-tournaments', 'previous-tournaments'), async (req, res) => {
    try {
      const { getTournamentDetails } = await import('./src/etl/ingestion/tournamentIndexer.js');
      const details = await getTournamentDetails(parseInt(req.params.id));
      if (!details) return res.status(404).json({ ok: false, error: 'Tournament not found' });

      const { tournament, placements } = details;

      // Split placements into host and opponent sides
      // host = placement 'winner' or 'finalist' based on winnerPlayer match
      ...
      // If we can't split by player address, use raw_battle_data
      const rawData = tournament.rawBattleData;
      const hostHeroes = hostPlacements.map(p => p.snapshot);
      const opponentHeroes = opponentPlacements.map(p => p.snapshot);

      // Combat formula helpers (ported from dfk-combat-formulas.ts)
      function dimStat(val, avgLevel, A, B, C, D, E, dimCap) {
        const raw = (A * val + B * avgLevel + C) / (D * avgLevel + E);
      ...
      function simInitWinPct(profilesA, profilesB, samples = 5000) {
        // For team tournaments, aggregate initiative by taking the best hero's init
        let wins = 0;
        for (let i = 0; i < samples; i++) {
      ...
      const avgLevel = placements.length > 0
        ? placements.reduce((s, p) => s + (p.snapshot.level || 1), 0) / placements.length
        : 1;

      // Build profiles — if no snapshots, fall back to raw battle data heroes
      let hostProfiles = hostHeroes.map(h => heroProfile(h, avgLevel));
      let oppProfiles = opponentHeroes.map(h => heroProfile(h, avgLevel));

      // Fallback: use rawBattleData hero arrays
      if (hostProfiles.length === 0 && rawData?.hostHeroes) {
      ...
      if (hostProfiles.length === 0 || oppProfiles.length === 0) {
      ...
      const actualWinner = tournament.winnerPlayer
        ? (tournament.winnerPlayer.toLowerCase() === tournament.hostPlayer?.toLowerCase() ? 'host' : 'opponent')
        : null;
      ...
      console.error('[Tournament Predict] Error:', err);
      res.status(500).json({ ok: false, error: err.message });
    }
  });
  ...
  app.get('/api/admin/tournament/class-winrates', isAdminOrHasTab('dfk-tournaments'), async (req, res) => {
    try {
      ...
      const rows = await rawPg`
        WITH winner_classes AS (
          SELECT
            tp.tournament_id,
            hts.main_class as winner_class
          FROM tournament_placements tp
          JOIN hero_tournament_snapshots hts ON hts.placement_id = tp.id
          WHERE tp.placement = 'winner'
        ),
        finalist_classes AS (
          SELECT
            tp.tournament_id,
            hts.main_class as finalist_class
          FROM tournament_placements tp
          JOIN hero_tournament_snapshots hts ON hts.placement_id = tp.id
          WHERE tp.placement = 'finalist'
        ),
        matchups AS (
          SELECT
            w.winner_class,
            f.finalist_class,
            COUNT(*) as bouts
          FROM winner_classes w
          JOIN finalist_classes f ON f.tournament_id = w.tournament_id
          WHERE w.winner_class IS NOT NULL AND f.finalist_class IS NOT NULL
          GROUP BY w.winner_class, f.finalist_class
          HAVING COUNT(*) >= 5
        )
        SELECT
          winner_class,
          finalist_class,
          bouts,
          ROUND(100.0 * bouts / NULLIF(bouts + COALESCE((
            SELECT bouts FROM matchups m2
            WHERE m2.winner_class = matchups.finalist_class AND m2.finalist_class = matchups.winner_class
          ), 0), 0), 1) as win_pct
        FROM matchups
        ORDER BY bouts DESC
        LIMIT 100
      ...
      console.error('[Tournament Class Winrates] Error:', err);
      res.status(500).json({ ok: false, error: err.message });
    }
  });
  ...
  app.get('/api/admin/tournament/browse', isAdminOrHasTab('dfk-tournaments'), async (req, res) => {
    try {
      ...
      const limit = Math.min(parseInt(req.query.limit) || 50, 200);
      const offset = parseInt(req.query.offset) || 0;
      const format = req.query.format; // '1v1','3v3','6v6'
      const realm = req.query.realm;
      const gloryBout = req.query.glory_bout === 'true';
      const levelMin = req.query.level_min ? parseInt(req.query.level_min) : null;
      const levelMax = req.query.level_max ? parseInt(req.query.level_max) : null;
      const rarityMin = req.query.rarity_min ? parseInt(req.query.rarity_min) : null;
      ...
      if (format) { whereClause += ` AND format = $${paramIdx++}`; params.push(format); }
      if (realm) { whereClause += ` AND realm = $${paramIdx++}`; params.push(realm); }
      if (gloryBout) { whereClause += ` AND glory_bout = true`; }
      if (levelMin !== null) { whereClause += ` AND (level_min IS NULL OR level_min >= $${paramIdx++})`; params.push(levelMin); }
      if (levelMax !== null) { whereClause += ` AND (level_max IS NULL OR level_max <= $${paramIdx++})`; params.push(levelMax); }
      if (rarityMin !== null) { whereClause += ` AND (rarity_min IS NULL OR rarity_min >= $${paramIdx++})`; params.push(rarityMin); }
      if (signature) { whereClause += ` AND tournament_type_signature = $${paramIdx++}`; params.push(signature); }

      const countResult = await rawPg.unsafe(`SELECT COUNT(*)::int as total FROM pvp_tournaments ${whereClause}`, params);
      const rows = await rawPg.unsafe(
        `SELECT * FROM pvp_tournaments ${whereClause} ORDER BY tournament_id DESC LIMIT $${paramIdx} OFFSET $${paramIdx+1}`,
        [...params, limit, offset]
      );
  ...
  // ============================================================================
  // PVE HUNT BATTLE LOG & PER-ENCOUNTER ANALYSIS
  // ============================================================================

  // GET /api/admin/pve/hunt-battle-log?encounterId=ID
  app.get('/api/admin/pve/hunt-battle-log', isAdminOrHasTab('pve-hunts'), async (req, res) => {
    ...
    try {
      ...
      const { COMBAT_CONTRACTS, HUNTS_DIAMOND_ABI } = await import('./src/config/combatContracts.ts');
      ...
      const heroIds = args[3]?.map(id => id.toString()) || [];
      ...
      console.error('[PVE BattleLog] Error:', err.message);
  ...
  app.post('/api/admin/pve/hunt-encounter-analysis', isAdminOrHasTab('pve-hunts'), async (req, res) => {
    ...
    try {
      ...
      const enemyLabel = (enemyId || 'unknown').split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ');

      const hasTurns = turns && Array.isArray(turns) && turns.length > 0;
      const turnSummary = hasTurns
        ? `Turn-by-turn data (${turns.length} turns):\n` + turns.map(t =>
            `  Turn ${t.turnNumber}: ${t.actorSide === 'hero' ? `Hero slot ${t.actorSlot}` : `Enemy slot ${t.actorSlot}`} used ${t.skillId || 'basic attack'} -> ${(t.targets || []).map(tgt => `slot ${tgt.slot} (${tgt.hpBefore}->${tgt.hpAfter}, ${tgt.damage} dmg)`).join(', ')}`
          ).join('\n')
      ...
      console.error('[PVE Encounter Analysis] Error:', err.message);
