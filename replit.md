...
*   **Hero Genetics System**: Complete genetics decoding layer with two specialized decoders:
    *   **Stat Gene Decoder** (`gene-decoder.js`): Decodes bit-packed `statGenes` using 4-bit extraction for classes, professions, abilities, stat boosts, and elements. Extracts all 4 gene tiers (D, R1, R2, R3) for full breeding genetics. Maps abilities to official DFK combat names (Poisoned Blade, Clutch, Leadership, Heal, Resurrection, etc.).
    *   **Visual Gene Decoder** (`visual-gene-decoder.js`): Decodes `visualGenes` using Kai (base-32) conversion matching the official degenking library implementation. Provides hex color codes for hair/eyes/skin, named appendages (Cat Ears, Dragon Wings, Royal Crown), and labeled backgrounds (City, Forest, Mountains). Full D/R1/R2/R3 support for all 12 visual traits.
    *   **Integration Layer** (`hero-genetics.js`): Combines both decoders into unified hero genetics output. Used by garden optimizer to detect gardening gene bonuses (40% stamina reduction) across all 4 gene positions. Powers `/debug-hero-genetics` Discord command showing complete genetic breakdown.
