...
*   **DFK Tournaments — Live Arena Feed**: Real-time DFK PvP bout browser. Two tabs: (1) **Live** — queries DFK GraphQL directly, auto-refreshes every 30 seconds, shows open challenges (waiting for opponent), in-progress fights, and recent completed bouts with hero lineups and restrictions; (2) **History** — DB-backed browse of indexed completed bouts with filters (format, rarity, level, glory). Write-through saving: every live fetch automatically upserts completed bouts to `pvp_tournaments` + `hero_tournament_snapshots` in the background. Backend: `GET /api/admin/tournament/live` (30s server-side cache, proxies DFK GraphQL, calls exported `processBattle` for background DB persistence), plus the existing browse/predict/stats endpoints. `processBattle` is exported from `src/etl/ingestion/tournamentIndexer.ts`. Combat Pets on-chain verification: Multicall3 `isOnAuction` check runs 90 seconds post-startup for CRY and SUN realms, filtering stale subgraph listings.
