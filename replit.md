...
*   **PVE Hunt Tracker**: Admin page at `/admin/pve-hunts` (also `/user/pve-hunts` via tab permission `pve-hunts`) for monitoring active PVE hunt expeditions. Wallet-based lookup fetches on-chain hero data, groups heroes by hunt zone activity, shows party composition with hero detail drill-down, encounter history from `hunting_encounters` table, and AI-powered party analysis. Backend endpoints: `GET /api/admin/pve/live-hunts`, `GET /api/admin/pve/hunt-encounters`, `POST /api/admin/pve/hunt-ai-analysis`.
