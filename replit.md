...
*   **Hero Genetics System**: Modular genetics decoding layer (`hero-genetics.js`) that extracts dominant traits (class, subclass, profession) from DFK GraphQL API string fields. **Limitation:** DFK GraphQL API does not expose raw gene data (fields `genes`, `visualGenes`, `statGenes` don't exist), only decoded string fields (`mainClassStr`, `subClassStr`, `professionStr`), so recessive genes (R1/R2/R3) are marked as "Unknown". Genetics-aware hero indexing uses dominant profession for garden optimization bonus detection.
