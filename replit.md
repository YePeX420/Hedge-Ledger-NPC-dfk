...
*   **Hunt Battle Log Viewer**: A Firebase-backed turn-by-turn battle log reader for PVE hunts. Accepts a hunt ID (`chainId-huntId` format or full URL), fetches combat state from DFK's `combat-engine` Firestore database, displays live HP bars and mana bars per combatant (heroes vs enemies), shows all action descriptions from `attackOutcome.battleLog`, highlights HP changes turn-to-turn, and auto-refreshes every 3 seconds when `hasWinner === false`. No Chrome Extension required — pure server-side Firebase token exchange.
