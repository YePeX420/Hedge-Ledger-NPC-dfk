...
*   **Bargain Hunter Cache System**: Implemented background caching for Bargain Hunter pages to eliminate slow load times. Cache computes TS (Total Tier Score) efficiency for hero pairs from indexed tavern data. The `bargain_hunter_cache` table stores pre-scored top 1000 pairs per summon type. Cache auto-creates if missing, refreshes after tavern indexer completes, and can be manually triggered via POST `/api/admin/bargain-cache/refresh`. Optimization limits scoring to 600 cheapest heroes (~180k pairs) for fast computation (~45 seconds).
